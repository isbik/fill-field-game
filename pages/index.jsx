...
export default function Home() {
  ...
  const checkIfPlayerNear = (x, y) => {
    if (field[x][y - 1]?.player === currentPlayer) return true;
    if (field[x][y + 1]?.player === currentPlayer) return true;
    if (field[x - 1]?.[y]?.player === currentPlayer) return true;
    if (field[x + 1]?.[y]?.player === currentPlayer) return true;
  };
  ...
  const handlePassMove = () => {
    ...
    for (let i = 0; i < SIZE; i++) {
      for (let j = 0; j < SIZE; j++) {
        if (checkIfPlayerNear(i, j) && !field[i][j]?.player) {
          x = i;
          y = j;
        }
      }
    ...
    if (x !== undefined && y !== undefined) {
      let newField = [...field];
      newField[x][y].player = currentPlayer;
  ...
  const isAllowPut = useMemo(() => {
    ...
    // check if all areas if near current players if free
    for (let i = x; i < x + height; i++) {
      for (let j = y; j < y + width; j++) {
        if (j >= 0 && j <= SIZE - 1) {
    ...
    <div className="game">
      <div className="info" style={{ display: "flex", gap: 10 }}>
        <div className="scores">
          ...
          <div className="count">
            {Array.from({ length: players }).map((_, index) => (
              ...
              >
                P{index + 1}: {countTotal[index + 1] || 0}
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="roll-numbers">
          <span className="roll-number">{numbers[0]}</span>
          <span className="roll-number">{numbers[1]}</span>
          <span onClick={handlePassMove} className="roll-number passing">
            Пропустить
          </span>
          <span onClick={() => setIsStarted(false)} className="close">
            X
          </span>
        </div>
      </div>
      <div onClick={() => isAllowPut && handleMove()} className="board">
          ...
          <div className="row" key={rowIndex}>
            {(row || []).map((cell, cellIndex) => (
              <div
                ...
                style={{
                  height: `calc((100vh - ${80 + SIZE * 6}px)/${SIZE})`,
                }}
                className={clsx(
                  "cell",
                  PLAYERS[field[rowIndex][cellIndex]?.player],
                  {
